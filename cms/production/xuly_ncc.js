...
function loadLoaiHoa(page, record) {
    ...
    queryDataPost("../php/data_get_ncc.php", datasend, function (res) {
        console.log("res", res)
        var stt = 1;
        ...
        if (arr.length == 0) {
            ...
            for (var x in arr) {
                ...
                <td>${item.EMAILNCC}</td>
                <td class="d-flex justify-content-between">${item.SDTNCC}
                  <div class="thaotac ">
                    <button class="btn-danger btn-sua" data-malh="${item.MANCC}" data-tenlh="${item.TENNCC}" data-mota="${item.DIACHINCC}" data-email="${item.EMAILNCC}" data-sdt="${item.SDTNCC}"><i class="fa fa-pencil-square-o mr-1"> </i>Sửa</button>
                    <button class="btn-danger btn-xoa" data-malh="${item.MANCC}"><i class="fa fa-trash mr-1"> </i>Xóa</button>
                  </div>
                </td>
                </tr>
                `;
