...
function loadCbLoaiHoa() {
    ...
    //ajax
    queryDataGet("../php/QG/data_get_cb_loaihoa.php", datasend, function (res) {
        console.log(res)
        var arr = res.items;
...
function loadHoa(page, record) {
    ...
    queryDataPost("../php/QG/data_get_quocgia_vaobang.php", datasend, function (res) {
        console.log("res", res)
        var stt = 1;
        ...
        if (arr.length == 0) {
            ...
            for (var x in arr) {
                ...
                data = data + `
                    ...
                    <div class="thaotac">
                        <button class="btn-danger btn-sua"  data-mah="${item.MAQUOCGIA}" data-tenh="${item.TENQUOCGIA}" data-mota="${item.GIOITHIEUQUOCGIA}" data-anh="${item.ANHQUOCGIA}"  data-malh="${arr[x].MALOAIHOA}"><i class="fa fa-pencil-square-o mr-1"> </i>Sửa</button>
